...
# Path to the file directory. Set this to the path of your subject folder, the default is dhanishta, but you will still need to ch
# This needs to be the subject folder, replace all occurances of "Dhanishta" with your subject name, and then take a small screen shot similar to the file "Dhanishta_folder.png" for it to search for and rename it to your subject. To do this quickly in visual studio, simply press 'ctrl+H', this will bring up a search window, in the top box type dhanishta, in the bottom field type the name of your subject folder. This will replace every use of dhanishta in the code with your ubject. Just remember to update the image too!
nakshatra_file_path = "C:\\Users\\autotube\\Documents\\Dhanishta"  

#This will need to be custom set to the path of YOUR LOGO FILE
logo_path = "C:\\Users\\autotube\\Documents\\Dhanishta\\channel_logo"
...
def select_all():
    ...
    print("Pressed ctrl+a to select all videos, Morty!")
...
def click_icon_size_dropdown():
    ...
    while not drop_down_location:
        drop_down_location = pyautogui.locateOnScreen('images/file_preview_drop_down_image.png', confidence=0.9)
...
def type_project_name():
    ...
    This function simulates typing the project name 'dhanishta' in the appropriate field in the Movavi Video Editor
    interface.
...
def add_channel_logo():
    ...
    # Click the drop down arrow to resize the icons to extra large
    find_question_mark_icon_click_drop_down_to_the_left()
    click_extra_large_icon_button()

    # Click the folder icon to browse for the logo file
    click_folder_icon()

    # Navigate to the channel logo file path
    goto_channel_logo_path(logo_path)

    # Double-click the channel logo icon to select it
    double_click_channel_logo_icon(channel_logo_icon)
...
def double_click_channel_logo_icon(channel_logo_icon):
    """
    Double-click the channel logo icon.

    This function locates the channel logo icon on the screen and double-clicks it.

    Args:
        channel_logo_icon (str): The filename of the channel logo icon image.

    Returns:
        None
    """
    while True:
        # Attempt to locate the channel logo icon on the screen
        channel_logo_icon_image_location = pyautogui.locateOnScreen(channel_logo_icon, confidence=0.8)
        if channel_logo_icon_image_location:
            logo_center = pyautogui.center(channel_logo_icon_image_location)
            pyautogui.doubleClick(logo_center)
            print("Double clicked the channel logo...")
            break
        else:
            print("I have not seen the channel logo icon yet, hang on!")
            time.sleep(0.5)
