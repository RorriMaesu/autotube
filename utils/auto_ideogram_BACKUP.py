...
from .browser_utils import start_browser, goto_settings, click_change_button, click_nakshatra_folder, click_select_folder_button, new_tab, exit_tab, close_browser
...
# Searches for the 'Generate' button on the screen and clicks it. Yawn.
def gen_button_click():
    start_time = time.time()  # Get the clock ticking, time's a-wasting.
    
    # Infinite loop, because why make it easy?
    while True:
        print("Searching for gen_button_click.png...")
        
        # Using that trusty confidence of 0.7 again. Go big or go home.
        generate_button = pyautogui.locateOnScreen('images/gen_button_click.png', confidence=0.7)
        
        # Found it? Great. Click it and let's move on with our lives.
        if generate_button:
            ...
            break  # Exit the loop, our work here is done.
        
        # If you're still here after 300 seconds, you've got bigger problems.
        elif time.time() - start_time > 300:
            print("Timeout: Generate button not found within 300 seconds")
            return None  # Just give up, man.
        
        # Rest for a sec. It's hard work being this pointless.
        time.sleep(1)
...
def click_profile_image():
    ...
    while time.time() - start_time < timeout:
        ...
        if me_location:
            ...
            time.sleep(3)  # Wait for 3 seconds, because anticipation makes it sweeter.
