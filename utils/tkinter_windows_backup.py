...
class TkinterManager:
    ...
    def how_many_images(self):
        root = tk.Tk()
        root.title("How many images?")
        self.set_window_geometry(root, 400, 100)
        
        entry_label = tk.Label(root, text="Number of images to be animated")
        entry_label.pack()
        entry = tk.Entry(root)
        entry.pack()
        submit_button = tk.Button(root, text="Submit", command=lambda: self.on_submit(entry, root))
        submit_button.pack()
    ...
    def on_submit(self, entry, root):
        try:
            entered_number = int(entry.get())
            if entered_number > 0:
                self.user_input = entered_number
                root.destroy()
            else:
